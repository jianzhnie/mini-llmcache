...
**32B 结论**:最优配置 **chunk=256, L1=8GB(TTFT 加速 1.36×)**。规律:① chunk=256 是甜点——128 的 RETRIEVE 帧数翻倍、传输开销大;512/1024 命中粒度变粗、部分命中场景收益下降 ② L1=8GB 全面优于 16GB(大池在驻留更多条目后 eviction 与预取交互更频繁)。

**全矩阵总结(8B + 32B)**:

| 模型 | 最优配置 | TTFT 加速 | 备注 |
|---|---|---|---|
| 0.6B | chunk=256, L1=8GB(默认) | 0.8× | prefill 太便宜,无法 >1× |
| 8B | chunk=128, L1=8GB | 0.98× | 全 12 配置 <1× |
| **32B(TP2)** | **chunk=256, L1=8GB** | **1.36×** | 大模型 + 中 chunk 的甜点组合 |

> **推荐配置**:chunk-size=256(默认值已是)、L1=8-16GB;32B+ 模型才能体现缓存价值。
