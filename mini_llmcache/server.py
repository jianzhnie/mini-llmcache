...
class CacheServer:
    def __init__(self, bind_url: str, chunk_size: int, capacity_bytes: int,
                 l2s: list[L2Adapter] = ()):
    ...
    def keys_for(self, hashes: list[bytes], model: str,
                 ranks: range | list[int]) -> list[ChunkKey]:
...
def serve(bind_url: str, chunk_size: int, capacity_bytes: int,
          l2_specs: list[str] = ()) -> None:
