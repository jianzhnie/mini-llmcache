...
class Pipeline:
    ...
    def __init__(
        ...
        self.staging = torch.empty(3, chunk_nbytes, dtype=torch.uint8, device=device)
        self.cpu_bufs = [
            torch.empty(chunk_nbytes, dtype=torch.uint8, pin_memory=True)
            for _ in range(3)
        ]
        ...
        for buf in self.staging:
            ...
            for layer, kv in enumerate(kv_caches):
                ...
                views.append(
                    chunk[layer].view(kv.dtype).view(blocks_per_chunk, *per_block_shape)
                )
...
class KVTransfer:
    def __init__(
        self, kv_caches: list[torch.Tensor], block_size: int, chunk_size: int, rank: int
    ):
        self.rank = rank
        ...
        self.chunk_nbytes = self.num_layers * self.blocks_per_chunk * self.block_nbytes
        self.d2h = Pipeline(
            kv_caches, self.blocks_per_chunk, self.block_nbytes, self.chunk_nbytes
        )
        self.h2d = Pipeline(
            kv_caches, self.blocks_per_chunk, self.block_nbytes, self.chunk_nbytes
        )
            ...
            for i in range(n_chunks):
                ...
                with device_module.stream(pipe.kernel_stream):
                    ...
                    for layer, kv_cache in enumerate(self.kv_caches):
                        torch.index_select(
                            kv_cache, 0, ids, out=pipe.layer_views[buf][layer]
                        )
                ...
                with device_module.stream(pipe.copy_stream):
                    ...
                    pipe.cpu_bufs[buf].copy_(pipe.staging[buf], non_blocking=True)
    ...
    def from_host(
        self, chunks: list[bytes], block_ids: list[int], skip_blocks: int = 0
    ) -> float:
        ...
        with pipe.lock, device_module.device(self.device):
            ...
            for i, blob in enumerate(chunks):
                ...
                pipe.cpu_bufs[buf].numpy()[:] = np.frombuffer(blob, dtype=np.uint8)
                skip = skip_blocks if i == 0 else 0
                ids = block_ids_t[
                    i * self.blocks_per_chunk + skip : (i + 1) * self.blocks_per_chunk
                ]
                with device_module.stream(pipe.copy_stream):
                    pipe.copy_stream.wait_event(pipe.free[buf])
                    pipe.staging[buf].copy_(pipe.cpu_bufs[buf], non_blocking=True)
