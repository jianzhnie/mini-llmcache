...
| Qwen3-32B(TP2) | **16384t 完全重复** | 3.65 s | 4.18 s | 0.87× | 59/59 命中但每请求传输 3.8GB;8GB L1 仅容 ~2 请求,淘汰抖动使热请求反慢 |
...
3072t 时缓存大致打平(0.92-1.08×):每请求传输(~320MB)的成本约等于它省下的 prefill。更长的 prompt 并不能改善——16384t 每请求传输 3.8GB,8GB L1 只能容纳 ~2 个请求,淘汰抖动使热请求反而比冷请求慢(0.87×)。经统计口径存活下来的收益:**多轮增量(整体 1.44×,传输跨轮次摊薄)** 与**并发正确性(20/20 命中,但传输争用使单请求 TTFT 膨胀 ~2.8×)**。
