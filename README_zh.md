...
| **Qwen3-32B(TP2)** | **2560t 前缀 × 20 请求** | 1.05 s | 0.66 s | **1.0-1.6× TTFT** | 全部 L1 命中;随批次/后缀长度波动 |
...
| Qwen3-32B(TP2) | **16384t 完全重复** | 3.56 s | 2.53 s | **1.4×** | 59/59 L1 命中,输出逐字节一致 |
...
|---|---|---|---|---|
| 完全重复 3072t | 0.633s | 0.585s | 1.1× | 11/11 L1 命中,输出逐字节一致 |
| 共享前缀 3072t | 0.597s | 0.526s | 1.1× | 10/10 L1 命中 |
| 无复用(基线) | 0.570s | — | — | 冷启动参考 |
| 100 条 SQuAD(1KB 上下文) | 0.202s | 0.298s | 0.72× | 命中率 80/80(100%);短上下文传输主导 |
| 吞吐(20 条共享前缀) | 0.986s | 0.60s | **1.6× TTFT / 1.2× 墙钟** | 全部 L1 命中 |

单请求场景下收益随前缀长度上升(3072t→1.1×,16384t→1.4×);但有两个经实测的边界:多轮增量链条超过 ~16 chunks 后收益递减(传输随链条线性增长,而每轮省下的 prefill 固定);并发客户端共享同一前缀时发生传输争用(单请求 TTFT 膨胀 ~2.8×)。
